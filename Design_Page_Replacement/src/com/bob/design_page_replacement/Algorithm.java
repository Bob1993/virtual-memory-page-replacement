...
public class Algorithm {
	...
	public static int[] arrays= new int[]{3,4,2,6,4,3,7,4,3,6,3,4,8,4,6};
...
/*				Log.i("result", ""+arrays[i]);
	...
	{
		Toast.makeText(context, "缺页数为："+missing_page, Toast.LENGTH_SHORT).show();
		missing_page= 0;
		queue.clear();
	}
/*	
 * debug
	public static void show(int i)//显示当前队列中元素
	{
		String re= "";
		for (int a : queue) {
			re+= a+" ";
		}
		Log.i("result", "置换之前"+re);
		String ele= "";
		for (; i < length; i++) {
			ele+= arrays[i]+" ";
		}
